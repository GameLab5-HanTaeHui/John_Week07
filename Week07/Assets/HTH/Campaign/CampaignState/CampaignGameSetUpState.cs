...
namespace HTH.Campaign
{
    ...
    ///   3. 순수 난수           — 폴백.
    /// </summary>
    public class CampaignGameSetupState : IState
    {
        ...
        public CampaignGameSetupState(CampaignLoopStateMachine loopSM, CharacterRegistry characterRegistry,
            StageRoleConfig stageRoleConfig, StageSetupConfig setupConfig)
        {
            _loopSM = loopSM;
        ...
        public void Enter()
        {
            ...
            // ── 4. 구역 배치 적용 ─────────────────────────────────────────
            // ★ 강제퇴고/일반퇴고 모두 시드 기준 초기 Zone으로 리셋
            ...
            if (_loopSM.LoopCount == 0)
                Debug.Log("[GameSetupState] 첫 루프 — 전원 초기 Zone 배치");
            else if (isForcedExit)
                Debug.Log("[GameSetupState] 강제퇴고 — 전원 초기 Zone 리셋");
            else
                Debug.Log("[GameSetupState] 일반 퇴고 — 전원 초기 Zone 리셋");
