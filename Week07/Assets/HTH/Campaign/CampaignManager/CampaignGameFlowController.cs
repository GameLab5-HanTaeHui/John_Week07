...
namespace HTH.Campaign
{
    ...
    public class CampaignGameFlowController : SingletonMonobehaviour<CampaignGameFlowController>
    {
        ...
        private void Start()
        {
            ...
            // HandleLoopReset 완료 후 IsForcedExit 초기화
            // (GameSetupState.Enter()가 FireLoopReset 직후 호출되므로
            //  HandleLoopReset → IsForcedExit 읽기 → 초기화 순서 보장)
            _loopSM.OnLoopReset += () => IsForcedExit = false;

            var turnSM = GetTurnSM();
            if (turnSM != null)
            {
                // 턴 종료 진입 — 뷰 갱신 + 사망자 캡처 (루프 리셋 부활 처리용)
                turnSM.OnTurnEndEntered += (_, isLoopCondition) =>
                {
                    IsForcedExit = isLoopCondition; // 강제퇴고 여부 저장
        ...
        private void OnDestroy()
        {
            ...
            _loopSM.OnLoopReset -= HandleLoopReset;
            _loopSM.OnGameEnded -= HandleGameEnded;
            // ★ IsForcedExit 초기화 람다는 익명이라 해제 불가 — 씬 파괴 시 자동 정리
        }
        ...
        /// <summary>
        /// 직전 루프가 강제퇴고(엔비 사망)로 종료됐는지 여부입니다.
        /// CampaignGameSetupState에서 Zone 리셋 여부 결정에 사용합니다.
        /// FireLoopReset() 호출 직전에 설정되고, EnterLoopStart() 후 자동 초기화됩니다.
        /// </summary>
        public bool IsForcedExit { get; private set; }

        /// <summary>
        /// 이전 루프에서 캐릭터가 마지막으로 있던 Zone ID를 반환합니다.
        /// 일반 퇴고 시 Zone 유지에 사용합니다.
        /// </summary>
        public int GetPreviousZone(int characterId)
        {
            var gameState = _loopSM?.GameState;
            if (gameState == null) return 0;
            var ch = gameState.GetCharacterState(characterId);
            return ch?.CurrentZone ?? 0;
        }
