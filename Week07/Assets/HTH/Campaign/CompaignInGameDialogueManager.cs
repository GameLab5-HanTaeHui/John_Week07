...
{
    /// <summary>
    /// 캠페인 씬 전용 DialogueManager입니다.
    /// 기본모드 DialogueManager와 씬을 완전히 분리합니다.
    ///
    /// ─── 기본모드와의 차이 ───────────────────────────────────────────────────
    ///   참조 대상: CampaignGameFlowController (기본: GameFlowController)
    ///   HandleTurnEndEntered 콜백:
    ///     CampaignGameFlowController.FinishTurnEnd() 호출
    ///   HandleGameEndDialogueRequested 콜백:
    ///     CampaignGameFlowController.NotifyGameEndDialogueComplete() 호출
    ///
    /// ─── 캠페인 승리 처리 ────────────────────────────────────────────────────
    ///   승리 다이얼로그 완료 후 로비로 이동하지 않습니다.
    ///   WinState 유지 → 캐릭터 이동/조각 수집 루프 계속.
    ///   씬 전환은 CampaignModeManager.OnAllCharactersCompleted()가 담당합니다.
    ///
    /// Canvas 구조 (씬에 직접 구성):
    ...
    /// </summary>
    public class CampaignInGameDialogueManager : MonoBehaviour
    {
        [Header("설정 에셋")]
        ...
        private void Start()
        {
            ...
            if (gfc == null)
            {
                Debug.LogError("[CampaignDialogueManager] CampaignGameFlowController를 찾을 수 없습니다.");
                enabled = false;
                return;
            }
            ...
            if (_turnSM == null)
            {
                Debug.LogError("[CampaignDialogueManager] TurnStateMachine을 가져올 수 없습니다.");
                enabled = false;
                return;
            }
        ...
        private void HandleGameEndDialogueRequested(bool isWin)
        {
            ...
            {
                // ★ CampaignGameFlowController 참조
                CampaignGameFlowController.Instance.NotifyGameEndDialogueComplete(isWin);
                return;
            }
            ...
            BeginPlay(() =>
            {
                _isGameEndDialogue = false;
                // ★ CampaignGameFlowController 참조
                CampaignGameFlowController.Instance.NotifyGameEndDialogueComplete(_pendingGameEndIsWin);
            });
        }
