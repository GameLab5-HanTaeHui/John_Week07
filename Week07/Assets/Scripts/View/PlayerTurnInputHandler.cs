...
/// 기본모드 전용입니다. 캠페인 모드는 CampaignPlayerInputHandler를 사용하세요.
///
/// 클릭  : 캐릭터 선택 → 같은 캐릭터 재클릭 → 대기 확정
/// 드래그 : 캐릭터를 잡고 Zone 위에 놓으면 이동 확정
...
public class PlayerTurnInputHandler : MonoBehaviour
{
    ...
    private void OnPress()
    {
        ...
        {
            Debug.Log($"[PlayerTurnInputHandler] 클릭 쿨타임 중 — ID:{charId}");
            return;
        }
    ...
    private void OnHold()
    {
        ...
        {
            _isDragging = true;
            GameFlowController.Instance?.BeginDragSelect(_draggingId);
        }
    ...
    private void OnRelease()
    {
        ...
        if (_isDragging)
        {
                ...
                && hit.collider.TryGetComponent(out ZonePoint zonePoint))
            {
                ...
                else
                {
                    ...
                    GameFlowController.Instance?.NotifyZoneClicked(zonePoint.ZoneId);
                }
            }
            else
            {
        ...
        else
        {
            var view = RaycastCharacter();
            if (view != null)
                GameFlowController.Instance?.NotifyCharacterClicked(view.CharacterId);
